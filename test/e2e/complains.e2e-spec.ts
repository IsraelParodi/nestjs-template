...
describe('[Feature] - Complains - /complains', () => {
  describe('Complains [POST /complains]', () => {
    ...
    it('should validate the existance of the LOV when create a complain', async () => {
      const customComplainDto = { ...baseComplainDto, currency: 100 };
      const response = await postRequest(BASE_URL, customComplainDto, 400);

      expect(response.body.error).toEqual({
        message: `Don't exist a LOV Detail with id: ${customComplainDto.currency} in LOV with key: currency`,
        error: 'Bad Request',
        statusCode: 400,
      });
    });
